...
@st.cache_data  # Added caching for data loading
def load_data(file_path: str) -> pd.DataFrame:
    """Load the dataset from the specified file path."""
    return pd.read_csv(file_path)

@st.cache_resource  # Added caching for model loading
def load_model(file_path: str):
    """Load the SVM model."""
    return joblib.load(file_path)

@st.cache_resource  # Added caching for vectorizer loading
...
if st.button("Process and Generate Articles"):
    if uploaded_data and uploaded_model and uploaded_vectorizer:
        try:
            with st.spinner('Processing data and generating articles...'):  # Added loading spinner
            ...
            st.success('Processing complete!')  # Added success message
