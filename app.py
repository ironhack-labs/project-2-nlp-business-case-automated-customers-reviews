...
# Constants (may not be needed if using file uploads)
...
def calculate_average_scores(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate average sentiment scores grouped
    by cluster and product name.
    """
...
def generate_article(category_name: str, top_products: List[str], bottom_product: str, reviews: str) -> str:
    ...
    Using the 'text-davinci-003' model for classic text completion.
    """
    prompt = f"""
