...
@st.cache_resource  # Cache the model loading step
def load_model(file_path: str):
    """Load the SVM model."""
    return joblib.load(file_path)

@st.cache_resource  # Cache the vectorizer loading step with _model to avoid hashing issues
def load_vectorizer(_model, file_path: str):
    """Retrieve the vectorizer from the model or a separate file."""
    if hasattr(_model, 'vectorizer'):
        return _model.vectorizer
    return joblib.load(file_path)
...
def generate_article(category_name: str, top_products: List[str], bottom_product: str, reviews: str) -> str:
    """Generate an article using OpenAI ChatCompletion API."""
    messages = [
...
if st.button("Process and Generate Articles"):
    if uploaded_data and uploaded_model and uploaded_vectorizer:
        try:
            with st.spinner('Processing data and generating articles...'):
